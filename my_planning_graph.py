...
class ActionLayer(BaseActionLayer):
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        for effect in actionA.preconditions:
            for affect in actionB.preconditions:
                if self.parent_layer.is_mutex(effect, affect): 
                    return True
        for effect in actionB.preconditions:
            for affect in actionA.preconditions:
                if self.parent_layer.is_mutex(effect, affect): 
                    return True   
        return False
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        for a in self.parents[literalA]:
            for b in self.parents[literalB]:
                if not self.parent_layer.is_mutex(a, b) and not self.parent_layer.is_mutex(b, a):
                    #print("inconsistent support", a, b, literalA, literalB)
                    flag = False
                    break
        return flag
...
class PlanningGraph:
    ...
    def h_maxlevel(self):
        ...
        for subgoal in self.goal:
            for i, layer in enumerate(self.literal_layers):
                if subgoal in layer:
                    max_level_sum = max(max_level_sum, i)
                    break
    ...
    def h_setlevel(self):
        ...
        def is_all_mutex(layer):
            for subgoalA, subgoalB in combinations(self.goal, 2):
                if subgoalA in layer and subgoalB in layer:
                        if layer.is_mutex(subgoalA, subgoalB):
                            return False
            return True
        ...
        for i, layer in enumerate(self.literal_layers):
            ...
            for g in self.goal:
                if g not in layer:
                    goal_reached = False
            if goal_reached and is_all_mutex(layer):
                return i
        return -1
